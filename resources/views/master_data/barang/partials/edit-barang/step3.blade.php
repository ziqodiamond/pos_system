<div x-data="satuanUkurData({{ $item->konversi ? json_encode($item->konversi) : '[]' }})" class="w-full">
    <!-- Header -->
    <div class="flex items-center gap-2 mb-4">
        <div class="w-6 h-6 bg-blue-100 rounded-full flex items-center justify-center">
            <svg class="w-4 h-4 text-gray-800" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                viewBox="0 0 24 24">
    ...
    <div>
        ...
        <div class="h-[190px] max-h-[190px] overflow-y-auto">
            <template x-for="(konversi, index) in konversis" :key="index">
                <div class="grid grid-cols-5 gap-2 mb-2 konversi-item overflow-visible">
                    <!-- Input Hidden ID Konversi -->
                    <input type="hidden" :name="'konversi_id[' + index + ']'" x-model="konversi.id">

                    <!-- Jumlah -->
                    <div>
                        <input type="number" name="jumlah[]" x-model.number="konversi.jumlah" readonly
                            class="w-full bg-gray-100 border border-gray-200 rounded px-2 py-1 text-sm" placeholder="1">
                    </div>

                    <!-- Satuan Konversi -->
                    <div x-data="dropdownEdit('satuanKonversi', konversi.satuan_konversi_id, index)" x-init="init()" class="relative">
                        <input type="text" x-model="search" @input="watchSearch()" @click="openDropdown()"
                            autocomplete="off" :placeholder="konversi.satuan_konversi_nama || 'Cari Satuan...'"
                        ...
                        <input type="hidden" :name="'satuan_konversi_id[' + index + ']'"
                            x-model="selectedSatuanKonversiId">
                    </div>

                    <!-- Nilai -->
                    <div>
                        <input type="text" :name="'nilai[' + index + ']'" x-model="konversi.nilai"
                            @input="formatNilai(index)"
                            class="w-full bg-gray-100 border border-gray-200 rounded px-2 py-1 text-sm"
                            placeholder="Nilai...">
                    </div>

                    <!-- Satuan Tujuan -->
                    <div x-data="dropdownEdit('satuanTujuan', konversi.satuan_tujuan_id, index)" x-init="init()" class="relative">
                        <input type="text" x-model="search" @input="watchSearch()" @click="openDropdown()"
                            autocomplete="off" :placeholder="konversi.satuan_tujuan_nama || 'Cari Satuan...'"
                            class="w-full bg-gray-100 border border-gray-200 rounded px-2 py-1 text-sm hover:placeholder-shown"
                        ...
                        <ul x-show="open" @click.outside="close()" x-cloak
                            ...
                            <li x-show="filteredSatuanTujuans.length === 0" class="px-2 py-1 text-gray-500 text-sm">
                                Data tidak ditemukan
                            </li>
                        </ul>
                        <!-- Input tersembunyi untuk menyimpan ID -->
                        <input type="hidden" :name="'satuan_tujuan_id[' + index + ']'"
                            x-model="selectedSatuanTujuanId">
                    </div>

                    <!-- Tombol Hapus -->
                    <div class="flex justify-center items-center">
                        <button type="button" @click="removeKonversi(index)"
                            class="text-red-500 hover:text-red-700 focus:outline-none"
                            :class="{ 'opacity-50 cursor-not-allowed': konversis.length <= 1 }"
                            :disabled="konversis.length <= 1" title="Hapus Konversi">
